...
H5 SDK 已设置为公开 scoped 包候选，但没有执行 npm publish。`network.request` 由 ArkTS Runtime 执行，H5 SDK 仍只负责 Bridge 请求和 callback 生命周期。H5 类型已由 JSON Manifest 镜像生成，后续仍需把 ArkTS Manifest、JSON、Markdown 与 H5 类型统一到真正的单一来源。
