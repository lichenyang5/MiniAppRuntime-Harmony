...
MyASCFRuntime 构造函数需要两个核心参数：



1. `WebviewController`  
   用于管理 ArkWeb 页面，并通过 `runJavaScript` 将 ArkTS 处理结果回调给 H5。

2. `Context`  
   表示当前宿主应用的运行环境。HAR 包本身是库模块，不能自己创建应用上下文，因此需要由 entry 或外部 Demo 通过 `getContext(this)` 传入。当前 Storage API 底层使用本地 Preferences 能力，需要依赖 Context 来定位应用沙箱和存储文件。
