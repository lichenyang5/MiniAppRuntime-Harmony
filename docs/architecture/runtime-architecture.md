...
[查看完整 Mermaid 架构图](../assets/diagrams/architecture-overview.md)
...
- `error/`：统一错误码和错误对象。
- `logger/`：统一 ArkTS 侧日志。
- `container/`：可复用的 Web 容器配置、URL Guard 和加载状态模型。
...
- Toast、Clipboard 和 Storage 三组真实 API。
